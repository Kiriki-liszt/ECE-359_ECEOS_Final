	...
	for (INT8U i = 0; i <= OS_LOWEST_PRIO - 1; i++) {
		...
		if ((OSTCBPrioTbl[i] != (OS_TCB*)0) && (OSTCBPrioTbl[i] != OS_TCB_RESERVED)) {
			...
			if (current->OSTCBDly == 0) {
				...
				// if : the deadline has passed cueent time tick
				// This must happen first to prevent false change of most urgent deadline
				if (current->deadline < (INT32S)OSTime) {
					// Using the same processing given at pdf page 19, the deadline update logic.
					current->deadline += current->period;
					fprintf(stderr, "%u\t%-17s%u%-27s\n", OSTime - 1, "Deadline fail : ", current->OSTCBPrio, " -> extend deadline to next period");
					continue;
				}

				// select a task with the closest deadline
				// if : currnet task in table has faster deadline than any before => than this is the most urgent 
				if (current->deadline < mostUrgentDeadline) {
					mostUrgentPriority = current->OSTCBPrio;    // this task is the most urgent
					mostUrgentDeadline = current->deadline;     // and this task has the fastest deadline
				}
			}
			////////////////////////////////////////////////////
		}
	}
	OSPrioHighRdy = mostUrgentPriority;                         // validate the most urgent priority
}
